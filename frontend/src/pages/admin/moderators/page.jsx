...
import { Plus, Edit2, Trash2, X, Image as ImageIcon, ShieldCheck, Shield, Search, ChevronLeft, ChevronRight, CheckSquare, Square, UserPlus } from 'lucide-react';
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2.5rem', paddingBottom: '2rem', borderBottom: '1px solid var(--border-color)' }}>
        <div>
          ...
          <p style={{ fontSize: '0.95rem', color: '#64748b', margin: 0 }}>
            Add, edit, or remove election moderators and staff.
          </p>
        </div>
        <button 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(1, 1fr)', gap: '1.25rem', marginBottom: '2.5rem' }}>
        <div className="dashboard-stat-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.85rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em', margin: 0 }}>Total Staff</p>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.75rem' }}>
            <span style={{ fontSize: '2.25rem', fontWeight: 800, color: '#0f172a', lineHeight: 1, letterSpacing: '-0.03em' }}>{moderators.length}</span>
          </div>
        </div>
      </div>
      ...
      {confirmDeleteId && createPortal(
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="dashboard-panel" style={{ width: '400px', padding: '2.5rem', animation: 'fadeIn 0.2s ease-out', borderRadius: '24px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.3)' }}>
            ...
            <h3 className="card-title mb-3" style={{ fontSize: '1.5rem', fontWeight: 800 }}>Remove Moderator?</h3>
            <p className="text-secondary mb-8 text-sm" style={{ lineHeight: 1.6 }}>Are you sure you want to remove this moderator? Their access will be revoked immediately.</p>
            <div className="flex gap-3 justify-center">
              ...
              <button className="btn" style={{ padding: '0.75rem 1.5rem', borderRadius: '12px', fontWeight: 600, background: 'var(--danger)', color: 'white', flex: 1 }} onClick={() => { deleteModerator(confirmDeleteId); setConfirmDeleteId(null); }}>Yes, Remove</button>
            </div>
          </div>
        </div>,
        document.body
