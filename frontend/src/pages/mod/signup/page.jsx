...
import { Users, Lock, Mail, ArrowLeft, ChevronRight, AlertCircle, User as UserIcon, UserPlus } from 'lucide-react';
...
export default function ModSignup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/auth/mod/signup', {
        method: 'POST',
    ...
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '2rem 1rem', overflow: 'hidden', backgroundColor: '#ffffff' }}>
      ...
      <div style={{ width: '100%', maxWidth: '440px', zIndex: 1, animation: 'fadeUp 0.8s ease-out', marginTop: '2rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '70px', height: '70px', borderRadius: '20px', background: '#f5f3ff', border: '1px solid #ddd6fe', marginBottom: '1rem', boxShadow: '0 10px 25px rgba(139, 92, 246, 0.1)' }}>
            <UserPlus size={32} color="#8b5cf6" />
          </div>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, letterSpacing: '-0.02em', marginBottom: '0.5rem', color: '#0f172a' }}>
            Create Mod Account
          </h1>
          <p style={{ color: '#64748b', fontSize: '1rem' }}>Register to monitor voting stations</p>
        </div>
        ...
        <div style={{ background: '#ffffff', borderRadius: '24px', padding: '2.5rem', border: '1px solid #e2e8f0', boxShadow: '0 20px 40px -15px rgba(0,0,0,0.05), 0 0 20px rgba(0,0,0,0.02)' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            {success && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#10b981', fontSize: '0.95rem', justifyContent: 'center', fontWeight: 600, background: 'rgba(16,185,129,0.1)', padding: '12px', borderRadius: '12px', border: '1px solid rgba(16,185,129,0.2)', animation: 'fadeUp 0.3s ease-out' }}>
            ...
            <button 
              ...
              style={{ width: '100%', height: '52px', marginTop: '0.5rem', fontSize: '1.05rem', fontWeight: 700, borderRadius: '12px', background: 'linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)', color: 'white', border: 'none', boxShadow: '0 10px 25px rgba(139,92,246,0.25)', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', transition: 'all 0.3s ease', cursor: loading ? 'not-allowed' : 'pointer' }}
              onMouseOver={(e) => { if(!loading) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 15px 35px rgba(139,92,246,0.35)'; } }}
              onMouseOut={(e) => { if(!loading) { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 10px 25px rgba(139,92,246,0.25)'; } }}
            >
            ...
            <div style={{ textAlign: 'center', marginTop: '1rem' }}>
              <Link to="/mod/login" style={{ fontSize: '0.9rem', color: '#8b5cf6', textDecoration: 'none', fontWeight: 600 }}>
